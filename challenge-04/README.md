...
carro.adicionarPessoas = function(numPessoas) {
  const totPessoas = carro.quantidadePessoas + numPessoas;
  if (carro.quantidadePessoas === carro.assentos && totPessoas >= carro.assentos) {
    return 'O carro já está lotado!';
  }
  if (totPessoas > carro.assentos) {
    const maxPessoas = carro.assentos - carro.quantidadePessoas;
    const pluralOuSingular = maxPessoas === 1 ? 'pessoa' : 'pessoas';
    return `Só cabem mais ${maxPessoas} ${pluralOuSingular}!`;
  }
