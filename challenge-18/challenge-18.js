(function () {
  ...
  */
  ...
  const allCPFs = ['049-214 3421-1', '210.458.522-05', '735 500 794 - 22', '101.123-131x32'];
  const formattedCPFs = allCPFs.map(function(cpf) {
  ...
  */
  ...
  formattedCPFs.forEach(function(cpf) {
    console.log(cpf.replace(regexCPF, '$1.$2.$3-$4'));
  });
  ...
  */
  ...
  console.log('<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'
    .replace(/<(\w+)>([^<]+)<\/\w+>/g, function (regex, tagName, value) {
      return `<${tagName}>O texto dentro da tag "${tagName}" é "${value}"</${tagName}>\n`;
    })
  );
})();
